...
| Chart | Description |
|---|---|
| `latency_bars.png` | p50/p95 grouped bar chart per strategy |
| `latency_cdf.png` | CDF curves — shows random's long tail clearly |
| `kv_hit_rate.png` | KV cache hit/miss pie (kv_aware only, real Redis hits) |
| `backend_distribution.png` | Request spread across backends per strategy |
| `latency_over_time.png` | Per-request latency — random spikes visible |
